...
unsigned int nTargetSpacing = 90; // 
unsigned int nStakeMinAge = 24 * 60 * 60; // 1 day
unsigned int nStakeMaxAge = 365 * 24 * 60 * 60; // 365 days
unsigned int nModifierInterval = 15 * 60; // time to elapse before new modifier is computed
...
const string strMessageMagic = "2X2 Signed Message:\n";
...
{
    int64_t nRewardCoinYear_1;
    //nRewardCoinYear_1 = MAX_MINT_PROOF_OF_STAKE; 

    int64_t nRewardCoinYear = 365 * CENT;

    int64_t nSubsidy = 0 * COIN;

    if (pindexBest->nHeight < 100000)
    {
      nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear * 1; // 365 %
      return nSubsidy + nFees;
    }  
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexLast->nHeight == 1150)
        return bnTargetLimit.GetCompact();  // force fork
...
// 2X2: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "2X2", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
//        block.nBits    = 0x1e0fffff;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd30c85ca0e242c72d26b1ed84974f201d8d430a0d8d2a8a8ed1e9572684e3b6b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd1, 0xc1, 0xc2, 0xa0 };
