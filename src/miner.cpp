...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ...
    std::string threadName = "NNSstake";
    ...
    while (true)
    {
        ...
        if (!regtestMode && !gArgs.GetBoolArg("-emergencystaking", false)) {
            while (connman->GetNodeCount(CConnman::CONNECTIONS_ALL) < MIN_STAKE_PEERS || ::ChainstateActive().IsInitialBlockDownload()) {
                pwallet->m_last_coin_stake_search_interval = 0;
                fTryToSync = true;
...
void StakeNNSs(bool fStake, CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool, boost::thread_group*& stakeThread)
{
